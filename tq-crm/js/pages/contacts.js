...
// Override saveContact to also push to Sheets
const _origSaveContact = saveContact;
// Patch persist calls in saveContact — push to sheets after local save
...
// Replace saveContact globally
window.saveContact = saveContactWithSync;

const _origDeleteContact = deleteContact;
async function deleteContactWithSync(id) {
  if (!confirm('Delete this contact and all associated data?')) return;
  contacts = contacts.filter(c => c.id !== id);
  estimates = estimates.filter(e => e.contactId !== id);
  tasks = tasks.filter(t => t.contactId !== id);
  ...
  renderContacts();
  toast('Contact deleted');
  pushDelete('Contacts', id);
}
window.deleteContact = deleteContactWithSync;
